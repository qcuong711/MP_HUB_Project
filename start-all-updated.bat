...
start "HRM App" cmd /k "cd /d \"%~dp0src\applications\hrm\" && npm run dev"

echo 🎉 Khởi động Event App (Port 3002)...
start "Event App" cmd /k "cd /d \"%~dp0src\applications\event\" && npm run dev"

echo 🏠 Khởi động Shell App (Port 3000)...
start "Shell App" cmd /k "cd /d \"%~dp0src\applications\gateway\" && npm run dev"
